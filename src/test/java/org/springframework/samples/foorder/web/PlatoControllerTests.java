...
class PlatoControllerTests {
	...
	@Test
	void testListarPlato() throws Exception {
	...
	void testGuardarPlato() throws Exception {
		...
		.andExpect(view().name("redirect:/platos?message=Guardado Correctamente"));
	}
	...
	void testDeletePlato() throws Exception {
		mockMvc.perform(get("/platos/delete/{platoId}",TEST_PLATO_ID)).andExpect(status().is3xxRedirection())
		.andExpect(view().name("redirect:/platos?message=Borrado correctamente"));
	}
	
	@WithMockUser(value = "spring")
	@Test
	void testDeletePlatoFalse() throws Exception {
		given(this.platoService.findById(TEST_PLATO_ID)).willReturn(Optional.empty());
		mockMvc.perform(get("/platos/delete/{platoId}",TEST_PLATO_ID)).andExpect(status().is3xxRedirection())
		.andExpect(view().name("redirect:/platos?message=Plato no encontrado"));
	}
	
	@WithMockUser(value = "spring")
	@Test
	void testDeletePlatoException() throws Exception {
		mockMvc.perform(get("/platos/delete/{platoId}",TEST_PLATO_ID)).andExpect(status().is3xxRedirection())
		.andExpect(view().name("redirect:/platos?message=Borrado correctamente"));
	}
