...
class ProveedorControllerTests {
	...
	@WithMockUser(value = "spring")
	@Test
	void testSaveProveedorSuccessFirstIf() throws Exception {
		List<String> listaNombres= new ArrayList<>();
		listaNombres.add("pepito");
		given(proveedorService.findAllNames()).willReturn(listaNombres);
		given(proveedorService.findByName(any(String.class))).willReturn(Optional.of(proveedor));
		mockMvc.perform(post("/proveedor/save")
				.param("name", "pepito")
				.param("gmail", "pepitopalotes@gmail.com")
				.param("telefono", "676661638").with(csrf()))
	...
	@WithMockUser(value = "spring")
	@Test
	void testSaveProveedorSuccessSecondIf() throws Exception {
		List<String> listaNombres= new ArrayList<>();
		listaNombres.add("pepito");
	...
	@WithMockUser(value = "spring")
	@Test
	void testPostProveedorEditadoSuccessFirstIf() throws Exception {
		List<String> listaNombres= new ArrayList<>();
		listaNombres.add("pepito");
		Proveedor prov= new Proveedor();
		prov.setName("nombreDistinto");
		given(proveedorService.findAllNames()).willReturn(listaNombres);
		given(proveedorService.findById(any())).willReturn(Optional.of(prov));
		mockMvc.perform(post("/proveedor/edit", TEST_PROVEEDOR_ID)
				.with(csrf())
				.param("id", "1")
				.param("name", "pepito")
				...
				.param("telefono", "666666666"))
		.andExpect(status().is3xxRedirection())
		.andExpect(view().name("redirect:/proveedor/edit/"+TEST_PROVEEDOR_ID+"?message=el proveedor ya existe"));
	}
